...
function cfwc_display_custom_field() {
	...
	if( $title ) {
		...
		printf(
			'<div class="cfwc-custom-field-wrapper"><label for="cfwc-title-field">%s</label><input type="number" id="cfwc-title-field" name="cfwc-title-field" value=""></div>',
			esc_html( $title )
		);
	}
...
function cfwc_validate_custom_field( $passed, $product_id, $quantity ) {
	// Product 1339 is buy container
	if ( $product_id == 1339 ) {
		if ( !isset( $_GET['container_id'] ) and empty( $_POST['cfwc-title-field'] ) ) {
			// Fails validation
			$passed = false;
			...
			if ( isset( $_GET['container_id'] ) ) {
				...
				$container_id  = $_POST['cfwc-title-field'];
			}
			// Check whether container is held by user
			global $wpdb;
			if ( is_user_logged_in() ){
				...
				$query = $wpdb->prepare("SELECT * 
										...
										, array( $container_id, 1 ));
				$result = $wpdb->get_row($query);
				if ( $result == null ){
					// User does not own container
					$passed = false;
					wc_add_notice( __( 'You do not own this container', 'cfwc' ), 'error' );
				}
			}
			else{
				// User not logged in
				$passed = false;
				wc_add_notice( __( 'User is not logged in', 'cfwc' ), 'error' );
			}
		}
	}
	return $passed;
...
function cfwc_add_custom_field_item_data( $cart_item_data, $product_id, $variation_id, $quantity ) {
	if( isset( $_GET['container_id'])) {
		// Add the item data
		$cart_item_data['title_field'] = $_GET['container_id'];
...
function cfwc_add_custom_data_to_order( $item, $cart_item_key, $values, $order ) {
	foreach( $item as $cart_item_key=>$values ) {
		if( isset( $values['title_field'] ) ) {
			$item->add_meta_data( __( 'container_id', 'cfwc' ), $values['title_field'], true );
		}
	}
}
