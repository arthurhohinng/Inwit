...
//Sets the container corresponding to a given container id to a given status.
function set_container_status($status, $cont_id){
    global $wpdb;
    ...
    if (is_user_logged_in()) { //check if user is logged in
        ...
        //Get a list of containers the user has ever been in possession of
        $wpdb->update('Container', array('status'=>$status), array('container_id'=>$cont_id, 'recipient_id'=>$user->id) );
